...
⚠️ **Nota importante**: Los recursos creados en este laboratorio serán utilizados en los Laboratorios 2 y 3 del workshop. NO elimine estos recursos al finalizar este laboratorio.
...
Su usuario de AWS debe tener permisos para:
...
# Debe mostrar su información de cuenta AWS
```
...
1. Verifique que está trabajando en la región correcta:
   - Confirme la región configurada ejecutando: `aws configure get region`
   - Si no es correcta, configure la región: `aws configure set region us-east-1`
...
El Refactor Set es el artefacto que orquesta la migración de recursos entre Stacks.

1. Ejecute el siguiente comando para crear el Refactor Set:
   ```bash
...
2. El comando retornará un ID del Refactor Set. Anótelo para el siguiente paso.
